...
class SASREC(tf.keras.Model):
    ...
    def __init__(self, **kwargs):
        ...
        self.max_hour = kwargs.get("max_hour", None)
        ...
        # modified recommend_embedding
        self.recommend_embedding = tf.keras.layers.Embedding(input_dim=self.max_hour + 1, output_dim=self.embedding_dim)
        self.dropout_layer = tf.keras.layers.Dropout(self.dropout_rate)
    ...
    def call(self, x, training):
        ...
        recommend_seq = x["recommend"]
        pos = x["positive"]
        neg = x["negative"]

        # add recommend embedding
        mask = tf.expand_dims(tf.cast(tf.not_equal(input_seq, 0), tf.float32), -1)
        seq_embeddings, positional_embeddings = self.embedding(input_seq)
        recommend_embeddings = self.recommend_embedding(recommend_seq)
        ...
        # masking
        seq_embeddings += recommend_embeddings
        seq_embeddings *= mask
    ...
    def create_combined_dataset(self, u, seq, pos, neg, rec):
        """
        ...
        rec = tf.keras.preprocessing.sequence.pad_sequences(
            rec, padding="pre", truncating="pre", maxlen=self.seq_max_len
        )
    ...
    def train(self, dataset, sampler, **kwargs):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for step in tqdm(
                ...
                u, seq, pos, neg, rec = sampler.next_batch()

                inputs, target = self.create_combined_dataset(u, seq, pos, neg, rec)
    ...
    def evaluate(self, dataset):
        ...
        print(dataset.user_train[0], dataset.user_valid[0], dataset.user_test[0])
        train = {i: [k[0] for k in j] for i, j in dataset.user_train.items()}  # removing deepcopy
        valid = {i: [k[0] for k in j] for i, j in dataset.user_valid.items()} 
        test = {i: [k[0] for k in j] for i, j in dataset.user_test.items()}
